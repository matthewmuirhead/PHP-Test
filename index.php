<!DOCTYPE html>
<!-- Matthew James Muirhead -->
<!-- Student Number: 16424341 -->
<html>
  <head>
    <meta charset="utf-8">
    <title>Matt Muirhead</title>
    <link href="https://fonts.googleapis.com/css?family=Montserrat" rel="stylesheet"><!-- Montserrat font -->
    <link href="https://fonts.googleapis.com/css?family=Roboto" rel="stylesheet"><!-- Roboto font -->
    <link rel="stylesheet" href="font-awesome/css/font-awesome.min.css"><!-- Font awesome CSS -->
    <link rel="stylesheet" href= "css/master.css"/><!-- CSS sheet -->
    <link rel="shortcut icon" href="images/favicon.png"/><!-- Favicon -->
  </head>
  <body id="homepage">
    <div class="header-homepage">
      <div class="horizontal-container-homepage">
        <div class="menu-container"><!-- Navigation menu -->
          <div class="menu">
            <div class="bar1"></div>
            <div class="bar2"></div>
            <div class="bar3"></div>
            <div class="nav-tree">
              <a href="index.html"><p class="home-button">Home</p></a>
              <a href="cv.html"><p class="cv-button">CV</p></a>
              <a href="bio.html"><p class="bio-button">Bio</p></a>
              <a href="contact.html"><p class="contact-button">Contact</p></a>
            </div>
          </div>
        </div>
        <div class="social-media-main"><!-- Social media buttons -->
          <h5>Find me online:</h5>
          <a href="https://www.linkedin.com/in/matthew-muirhead-856679134"><img src="images/linkedin.ico" alt="linkedin" class="social-media-images linkedin"></a>
          <a href="https://github.com/matthewmuirhead"><img src="images/GitHub.png" alt="GitHub" class="social-media-images github"></a>
          <a href="http://codepen.io/matthewmuirhead/"><img src="images/codepen.png" alt="Codepen" class="social-media-images codepen"></a>
          <a href="https://twitter.com/PTG_Muiry"><img src="images/twitter.png" alt="Twitter" class="social-media-images twitter"></a>
        </div>
      </div>

    </div>
    <div class="titles"><!-- Welcome text -->
      <img src="images/me.png" alt="me" class="picture-of-me"><!-- Picture of me -->
      <h1>Hello, I am Matt</h1>
      <div class="horizontal-container-homepage">
        <h4>I'm a student at the Univeristy of Northampton.</h4>
        <a href="bio.html" class="read-more horizontal-container-homepage"><h4>Read More</h4>
        <i class="fa fa-long-arrow-right arrow-adjust" aria-hidden="true"></i></a>
      </div>
    </div>
  </body>
</html>
